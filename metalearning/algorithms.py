...
Implements MAML, Reptile, ANIL, and related algorithms for SNNs.
"""
...
class MAML(nn.Module):
    ...
    def adapt(self,
        ...
        def functional_forward(x, params, buffers):
            return torch.func.functional_call(self.model, (params, buffers), x)

        for step in range(steps):
            x, y = train_data
            logits = functional_forward(x, params, buffers)
...
class ANIL(nn.Module):
    ...
    def adapt(self,
        ...
        for step in range(steps):
            ...
            # Forward with fast head + fixed body using functional call
            params = OrderedDict()
            params.update(body_state)
            params.update(fast_head)
            buffers = OrderedDict(self.model.named_buffers())

            logits = torch.func.functional_call(self.model, (params, buffers), x)
            loss = loss_fn(logits, y)
    ...
    def meta_update(self, tasks, loss_fn):
        ...
        init_body = {name: param.clone() for name, param in self.body_params}

        adapted_heads = []
        for train_x, train_y, test_x, test_y in tasks:
            adapted = self.adapt(loss_fn, (train_x, train_y))
            head_only = {k: v for k, v in adapted.items() if k in dict(self.head_params)}
            adapted_heads.append(head_only)
...
class SNNMetalearner:
    ...
    def _run_episode(self, task: Dict, snn) -> float:
        ...
        # For rate-coded SNN:
        if hasattr(snn, 'run') and not hasattr(snn, 'neuron_group'):
            # Rate-coded SNN interface
            if 'sensory_input' in env_config:
                snn.set_sensory_input(env_config['sensory_input'])
            snn.run(env_config.get('duration', 100.0))
            motor_output = snn.get_motor_output()
            decoded = task['motor_decoding'](motor_output)
            reward = task['reward_fn'](decoded, env_config)
            return reward

        # For spiking SNN (Brian2):
        if hasattr(snn, 'neuron_group') and hasattr(snn, 'inject_sensory_input'):
            sensory_rates = env_config.get('sensory_rates', {})
            duration = env_config.get('duration', 100.0)
            snn.inject_sensory_input(sensory_rates, duration)
            motor_output = snn.get_motor_output(duration)
            decoded = task['motor_decoding'](motor_output)
            reward = task['reward_fn'](decoded, env_config)
            return reward
    ...
    def _inner_adapt(self, task: Dict, snn) -> Any:
        ...
        """
        # For rate-coded SNN with learnable weights
        if hasattr(snn, 'W') and hasattr(snn, 'rates'):
            # Gradient-based adaptation on W
            inner_model = deepcopy(snn)
            inner_opt = torch.optim.SGD([inner_model.W], lr=self.metalearning_config.inner_lr)

            for step in range(self.metalearning_config.inner_steps):
                inner_opt.zero_grad()
                # Run episode and compute loss
                episode_reward = self._run_episode(task, inner_model)
                loss = -episode_reward
                loss.backward()
                inner_opt.step()
